...
**Where things stand:** 757 Python tests green on 3.10 and 3.12, plus 130
...
| **The sync API is authenticated (AUDIT SF-09)** | The device ID alone used to be the credential, and one legacy code path put it straight in a URL, where it lands in server, proxy and log lines. Registration now also hands back a bearer token (`RegisterResp.t`) that every later request sends as `Authorization: Bearer`. Also found and fixed while closing this: `ui/devices_page.py` and `ui/processes_page.py` each carried their own hand-rolled, unauthenticated request code — one hit a `/r` endpoint nothing else agrees on, another put the device ID in a URL path — instead of going through the tested `core/syncclient.py`/`core/linking.py`. Both now delegate to it. The server-side check still waits on #8 below; the client sends the header regardless. A review pass afterward caught the fix undoing itself in the most common case: `core/syncclient.py`'s `SyncClient` is built once at startup and its background thread keeps using that same instance, so a token issued by registering *while the app is already running* — the only way anyone registers — was never picked up until a restart. `_transport` is a property now, rebuilt from current config on every use unless a transport was injected (tests). The Android scanner had the sibling bug — joining right after an in-flow auto-registration reused a client whose transport predated the token — fixed by rebuilding the client after registration succeeds, before the join call. |
...
| Done | What it means |
|---|---|
| **887 tests** | 757 Python across 3.10 and 3.12, 130 Kotlin. Plus lint, a byte-compile pass over the Tk modules the suite cannot import, and a packaging-config check. |
