...
| SF-09 | *Partial* | Registration now returns a per-device secret (`server/models.py` `RegisterResp.tok`), stored and sent as `Authorization: Bearer <token>` on every call after — the device id was never the credential it was treated as: it travels in every payload already and is shown back to the user in the Devices tab. `ui/devices_page.py`'s group-membership fetch no longer carries the id in the URL either. Also removed a second, undocumented poller in `ui/processes_page.py` — marked `(legacy)` in its own code — that queried `GET /sync/{device_id}` directly on its own 15-minute timer, independent of the audited sync client and reproducing this finding verbatim. **Still needed:** a server that actually checks the token; this is client-side only until #8 in `STATUS.md` (server in version control) lands |
