"""
launcher.py - Starting ProtBot: services, window, tray, and the quit path.

This was `main.py` at the repository root, and it opened by pushing its own
directory onto the front of `sys.path` so that `core` and `ui` could be
imported. That line was doing nothing — Python already puts a script's own
directory there, whatever the working directory — and it was standing in for
the thing that was actually missing, which was any packaging metadata at all
(AUDIT ST-04). The project declares a build backend and a `protbot` entry
point in pyproject.toml now, and this module is what that entry point calls.

`main.py` is still there, as a three-line shim. The PyInstaller spec, the
`.bat` launcher and BUILD.md all name it, and there is no reason to break
three things to move one function.
"""
...
def create_tray_icon(root: tk.Tk, quit_fn):
    ...
    """
    # One level up from ui/, which is where the icon lives in a source tree.
    # In a frozen build PyInstaller flattens the bundled data files next to
    # the executable, so both candidates are tried rather than assuming.
    here = os.path.dirname(os.path.abspath(__file__))
    icon_path = ""
    for candidate in (os.path.join(os.path.dirname(here), "ProtBot.ico"),
                      os.path.join(getattr(sys, "_MEIPASS", here), "ProtBot.ico")):
